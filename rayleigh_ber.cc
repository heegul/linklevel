...
NS_LOG_COMPONENT_DEFINE("LteOfdmaSimulation");

// LTE OFDM parameters
const int NUM_SUBCARRIERS = 1200; // 100 Resource Blocks (RBs), 12 subcarriers per RB
const int CP_LENGTH_NORMAL = 144; // Normal cyclic prefix length in LTE
const double SUBCARRIER_SPACING = 15000; // 15 kHz subcarrier spacing
const double SYMBOL_DURATION = 1 / SUBCARRIER_SPACING;
const double TOTAL_SYMBOL_DURATION = SYMBOL_DURATION + (CP_LENGTH_NORMAL * SYMBOL_DURATION / NUM_SUBCARRIERS);

// Pilot structure
const int FREQ_PILOT_SPACING = 6; // LTE pilot spacing in frequency domain
const int TIME_PILOT_SPACING = 7; // LTE pilot spacing in time domain

// Frame structure
const int SYMBOLS_PER_FRAME = 14; // 14 OFDM symbols per subframe in LTE
...
{
    std::vector<std::complex<double>> output(NUM_SUBCARRIERS + CP_LENGTH_NORMAL);
    ...
    // Add cyclic prefix
    std::copy(ifft_out.end() - CP_LENGTH_NORMAL, ifft_out.end(), output.begin());
    std::copy(ifft_out.begin(), ifft_out.end(), output.begin() + CP_LENGTH_NORMAL);
...
std::vector<std::complex<double>> OfdmDemodulate(const std::vector<std::complex<double>>& input)
{
    ...
    std::copy(input.begin() + CP_LENGTH_NORMAL, input.end(), fft_in.begin());
...
std::vector<std::complex<double>> EstimateChannel(const std::vector<std::vector<std::complex<double>>>& receivedSymbols, const std::vector<std::vector<std::complex<double>>>& pilotSymbols)
{
    ...
    // Perform spline interpolation for more accurate channel estimation
    for (size_t f = 0; f < NUM_SUBCARRIERS; ++f)
    {
        if (!pilotPositions[f])
        {
            size_t leftPilot = f;
            while (leftPilot > 0 && !pilotPositions[leftPilot])
                --leftPilot;
            size_t rightPilot = f;
            while (rightPilot < NUM_SUBCARRIERS - 1 && !pilotPositions[rightPilot])
                ++rightPilot;
...
int main(int argc, char *argv[])
{
    ...
    LogComponentEnable("LteOfdmaSimulation", LOG_LEVEL_INFO);
